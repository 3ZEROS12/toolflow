...
1. **按阶段组装工具，有条不紊**：它先把你的大活拆成阶段。第一步调研，只把搜索和抓取工具拿出来，把写权限锁死；第二步核心编码，把外面无关的重型 MCP 全屏蔽掉，只留编辑器和本地运行时，模型注意力高度集中，绝不选错工具；第三步走查，立马物理切断写入权限，模型想改都改不了，只负责客观挑刺。
2. **长日志秒级脱水，守住上下文**：跑测试或构建炸出五百行日志？它毫秒级自动给你落盘封存，塞给模型的只有核心退出码、错误摘要和文件路径。你那几万 Token 就是这么保住的。
3. **自带开箱即用的提示词工作台**：按 `[p]` 就能从历史常用模板里挑一个直接填入，按 `[+]` 还能当场写一段，按 `Ctrl+L` 模型还能帮你自动提炼标签。再也不用到处翻剪贴板了。”
